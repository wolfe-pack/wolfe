...
object GridExample {
  ...
  def main(args: Array[String]) {
    ...
    def vertical(y: Image) = sum(0 until n - 1) { i => sum(0 until n) { j => l_g * I(y(i, j) == y(i + 1, j)) } }
    def horizontal(y: Image) = sum(0 until n) { i => sum(0 until n - 1) { j => l_g * I(y(i, j) == y(i, j + 1)) } }
