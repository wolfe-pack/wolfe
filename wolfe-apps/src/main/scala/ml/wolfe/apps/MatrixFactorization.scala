...
object MatrixFactorization extends App {
  ...
  db.sampleTensor(10, 10, 0, 0.2) //samples a matrix
  ...
  for (d <- data) {
    val (colIx, rowIx, _) = db.getKey(d.ix)
    val a = A(rowIx)
    val v = V(colIx)

    //create positive fact factor
    fg.buildFactor(Seq(a, v))(_ map (_ => new VectorMsgs)) { 
      e => new CellLogisticLoss(e(0), e(1), 1.0, 0.05) with L2Regularization
    }
    ...
    fg.buildStochasticFactor(Seq(v, db.sampleNode(colIx)))(_ map (_ => new VectorMsgs)) {
      e => new CellLogisticLoss(e(0), e(1), 0.0, 0.05) with L2Regularization
    }
  }
