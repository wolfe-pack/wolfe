...
case class Cell(key1: Any, key2: Any = DefaultIx, key3: Any = DefaultIx, target: Double = 1.0, cellType: CellType = Train) {
...
trait Formula {
  ...
  def apply(key: Any)(implicit db: KB, state: State): Double
}
...
case class Impl(p1: Any, p2: Any, target: Double = 1.0) extends Formula2(p1, p2) {
  override def apply(key: Any)(implicit db: KB, state: State): Double = db.prob(p1, key) * (db.prob(p2, key) - 1) + 1
  override def toString: String = p1 + " => " + p2
}

case class ImplNeg(p1: Any, p2: Any, target: Double = 1.0) extends Formula2(p1, p2) {
  override def apply(key: Any)(implicit db: KB, state: State): Double = db.prob(p1, key) * -db.prob(p2, key) + 1
  override def toString: String = p1 + " => !" + p2
}
...
class KB(k: Int = 100) extends Tensor {
  /**
  ...
  val ix1ToVarMap = new mutable.HashMap[CellIx, VectVar]()
  val ix2ToVarMap = new mutable.HashMap[CellIx, VectVar]()
  val ix3ToVarMap = new mutable.HashMap[CellIx, VectVar]()
  //for pair-embeddings
  val ix23ToVarMap = new mutable.HashMap[(CellIx, CellIx), VectVar]()
  ...
  def getPredictedBy1(key: CellIx, threshold: Double = 0.5)(implicit state: State) = keys2.filter(this.prob(key, _) >= threshold).map((_, DefaultIx))
  def getPredictedBy2(key: CellIx, threshold: Double = 0.5)(implicit state: State) = keys1.filter(this.prob(key, _) >= threshold).map((_, DefaultIx))
  //fixme: currently only applicable to matrices
  def getPredictedBy3(key: CellIx, threshold: Double = 0.5)(implicit state: State) = ???
  def getPredictedBy23(key1: CellIx, key2: CellIx, threshold: Double = 0.5)(implicit state: State) = ???

  def ++=(cells: Seq[Cell]) = cells foreach (this += _)

  def var1(key1: CellIx) = ix1ToVarMap.get(key1)
  def var2(key2: CellIx) = ix2ToVarMap.get(key2)
  def var3(key3: CellIx) = ix3ToVarMap.get(key3)
  def var23(key23: (CellIx, CellIx)) = ix23ToVarMap.get(key23)

  //fixme: need to keep a setting that maps VectVar to an actual FactorieVector
  def vector1(key1: CellIx)(implicit state: State) = var1(key1).map(_.asInstanceOf[VectVar]).map(state(_))
  def vector2(key2: CellIx)(implicit state: State) = var2(key2).map(_.asInstanceOf[VectVar]).map(state(_))
  def vector3(key3: CellIx)(implicit state: State) = var3(key3).map(_.asInstanceOf[VectVar]).map(state(_))
  def vector23(key23: (CellIx, CellIx))(implicit state: State) = var23(key23).map(_.asInstanceOf[VectVar]).map(state(_))
  ...
  def toVariables: Seq[VectVar] = {
    if (isMatrix) {
      ix1ToVarMap ++= keys1 map (key => key -> new VectVar(k, key.toString))
      ix2ToVarMap ++= keys2 map (key => key -> new VectVar(k, key.toString))
    } else ???
    ...
    rows.foreach(row => {
      ...
      cols.foreach(col => {
        ...
        if (showTrain) {
          ...
          val colVec = vector1(col).get //ix1ToVarMap(col).variable.asVector.b
          val rowVec = vector2(row).get //ix2ToVarMap(row).variable.asVector.b
          val p = sigmoid(rowVec dot colVec)
  ...
  @deprecated
  def writeVectors(filePath: String)(implicit state: State) = {
    val writer = new FileWriter(filePath)
    (ix1ToVarMap ++ ix2ToVarMap).foreach(p => {
      val (name, variable) = p
      val vec = state(variable.asInstanceOf[VectVar])
  ...
  @tailrec
  final def sampleNodeFrom2(key1: CellIx, attempts: Int = 1000, sampleTestRows: Boolean = true): VectVar = {
    if (isMatrix)
      if (attempts == 0) {
        println("WARNING: Could not find negative cell 2 for key1: " + key1.toString)
        ix2ToVarMap(keys2(random.nextInt(keys2.size)))
      } else {
  ...
  @tailrec
  final def sampleNodeFrom1(key2: CellIx, attempts: Int = 1000): VectVar = {
    if (isMatrix)
      if (attempts == 0) {
        println("WARNING: Could not find negative cell 1 for key2: " + key2.toString)
        ix1ToVarMap(keys1(random.nextInt(keys1.size)))
      } else {
   ...
   */
  def serialize(pathToDir: String)(implicit state: State) = {
    ...
    def writeVectors(ixToNodeMap: mutable.HashMap[CellIx, VectVar], writer: FileWriter): Unit = {
      ixToNodeMap.foreach(t => {
        val (ix, node) = t
        val vec = state(node.asInstanceOf[VectVar])
        writer.write(ix + "\t" + Serialization.write(vec) + "\n")
      })
      writer.close()
    }

    writeVectors(ix1ToVarMap, new FileWriter(dir.getAbsolutePath + "/rows.txt"))
    writeVectors(ix2ToVarMap, new FileWriter(dir.getAbsolutePath + "/cols.txt"))
  }

  def deserialize(pathToDir: String): State = {
    implicit val formats = Serialization.formats(NoTypeHints)
    ...
    val rows = Source.fromFile(dir.getAbsolutePath + "/rows.txt").getLines().map(l => {
      val Array(name, vec) = l.split("\t")
      ix1ToVarMap(name).asInstanceOf[VectVar] -> new DenseTensor1(Serialization.read[Array[Double]](vec))
    })

    val cols = Source.fromFile(dir.getAbsolutePath + "/cols.txt").getLines().map(l => {
      val Array(name, vec) = l.split("\t")
      ix2ToVarMap(name).asInstanceOf[VectVar] -> new DenseTensor1(Serialization.read[Array[Double]](vec))
    })

    new MapBasedState((rows ++ cols).toMap)
  }
}

/*
trait Features extends KB {
