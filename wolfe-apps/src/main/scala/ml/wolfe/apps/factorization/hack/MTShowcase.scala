...
import ml.wolfe.apps.factorization.Formula
import ml.wolfe.apps.factorization.Impl
import ml.wolfe.apps.factorization.ImplNeg
import ml.wolfe.apps.factorization.TensorKB
import ml.wolfe.apps.factorization.util.FormulaeExtractor._
...
/**
 * Finds strongest connections between portuguese shallow textual patterns and english dependency path patterns.
 * @author rockt
 */
object MTShowcase extends App {
  type Rule = Formula
  type Entity = Any
  type Relation = Any
  type SPDB = TensorKB
  ...
  println("Loading db...")
  db.deserialize(args.lift(0).getOrElse("wolfe-apps/data/out/bbc/serialized/"))
  println(db.toInfoString)

  val premises = db.relations.filter(_.toString.startsWith("por"))
  //val consequents = db.relations.filter(consequentFilter)
  val consequents = db.relations.filter(_.toString.startsWith("eng"))
  ...
  val ruleWriter = new FileWriter("wolfe-apps/data/formulae/mt.txt")
  potentialRules
  //.filter(_._2 >= 0.9)
  .filter(_._3 >= 10)
  .sortBy(-_._1)
