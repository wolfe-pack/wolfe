...
import ml.wolfe.legacy.term.Target
import ml.wolfe.legacy.term.ValueOutsideOfDomain
import scala.Some
import ml.wolfe.legacy.value.Gen
import ml.wolfe.legacy.term.FunctionNotDefinedAt
import ml.wolfe.legacy.term.Constant
import org.scalautils.Bad
import ml.wolfe.legacy.term.VariableUndefined
import cc.factorie.la.Tensor1
import ml.wolfe.{MaxProduct, Index}
import ml.wolfe._
...
class ExperimentalSpecs extends WordSpec with Matchers {
  ...
  def gradientBasedMinimizer(newMinimizer: => (LambdaAbstraction[FactorieVector,Double] => Term[State])) {
    ...
    "find a minimum of a perceptron loss" in {
      ...
      val gold = state(i -> 2)
      val objective = max(lam(i, model)).byBruteForce - (model | gold)  
      val result = newMinimizer(lam(weights,objective))
  ...
  def perceptronMinimizer = min(_: LambdaAbstraction[FactorieVector,Double]).byTrainer(new OnlineTrainer(_, new Perceptron, 5)).argminState

  "Perceptron Minimizer" should {
    behave like gradientBasedMinimizer(perceptronMinimizer)
  }
...
object CustomEqualities {
  ...
  implicit val vectorEq = new Equality[FactorieVector] {
    def areEqual(a: FactorieVector, b: Any): Boolean = (a, b) match {
      case (v1: SparseVector, v2: SingletonVector) => v1.activeDomain.forall(i => v1(i) === v2(i) +- eps)
      case (v1: FactorieVector, v2: FactorieVector) => v1.length == v2.length && v1.activeDomain.forall(i => a(i) === v1(i) +- eps)
      case _ => false
    }
  }

  implicit val vectorEqIgnoreLength = new Equality[FactorieVector] {
    def areEqual(a: FactorieVector, b: Any): Boolean = (a, b) match {
      case (v1: FactorieVector, v2: FactorieVector) => v1.activeDomain.forall(i => a(i) === v1(i) +- eps)
      case _ => false
    }
  }
}