...
import ml.wolfe.MPGraph.FactorType
import ml.wolfe.legacy.TermDSL._
import scala.Unit
import ml.wolfe.legacy.value.{Doubles, Bools}
import ml.wolfe.{MaxProduct, DualDecomposition, MPGraph}
...
class MAPInferenceSpecs extends WordSpec with Matchers {
  ...
  def buildFactorTable(states: Array[Array[Int]]): MPGraph = {
    ...
    // TODO SR: you should call build on graph after done with adding nodes and factors
    f.edges = edges
