...
import ml.wolfe.legacy.TermConverter._
import ml.wolfe.legacy.value.{Fun, Vectors}
import ml.wolfe.legacy._
import scala.Some
import ml.wolfe.{MaxProduct, FactorieVector, SparseVector, DenseVector, MPGraph}
import TermDSL._
import ml.wolfe.ilp.MPwithILP

object MultiVariate {
  def unapply(term: Term[Double]): Option[Variable[FactorieVector]] = term match {
...
trait MultiVariate extends Term[Double] {
  def parameter: Variable[FactorieVector]
}
...
trait Differentiable extends MultiVariate {
  def gradient: Term[FactorieVector]
}

object Max {
  def unapply(term: Term[Double]): Option[(Variable[FactorieVector], Term[FactorieVector], Term[State])] = term match {
  ...
  case class ByBruteForce[T](term: LambdaAbstraction[T,Double]) extends Max[T] with Composite1[Fun[T,Double], Double] with DoubleTerm {
    ...
    private var conditionedCoefficient: Term[FactorieVector] = _
...
object Differentiator {
  ...
  def differentiate(param: Variable[FactorieVector], term: Term[Double]): Option[Term[FactorieVector]] = {
    term match {
 ...
 */
class UnusedParameter extends Variable[FactorieVector] {
  def domain[C >: ml.wolfe.FactorieVector] = Constant(Vectors).asInstanceOf[Term[Set[C]]]
...
trait MaxHint extends CompilerHint {
  ...
  type ArgmaxValueGradient[T] = (Term[T], Term[Double], Term[FactorieVector])
  // should have no free variables
  def withoutParam[T](sig: Sig[T], term: Term[Double]): Max2[T]
  //should have one free variable: param
  def withParam[T](sig: Sig[T], param: Variable[FactorieVector], term: Term[Double]): Max2[T] with Differentiable
}
...
case class MessagePassingHint(algorithm: MPGraph => Unit = MaxProduct.apply(_, 1)) extends MaxHint {
  ...
  def withParam[T](sig: Sig[T], param: Variable[ml.wolfe.FactorieVector], term: Term[Double]) = {
