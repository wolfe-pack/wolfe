...
object Untyped {
  ...
  object FunApp {
    ...
    def unapply(t: Term[Any]) = t match {
      case ml.wolfe.legacy.term.FunApp(f, a) => Some(f.asInstanceOf[Term[Fun[Any, Any]]], a.asInstanceOf[Term[Any]])
      case _ => None
    }
  }
  ...
  object TupleTerm2 {
    ...
    def unapply(t: Term[Any]) = t match {
      case ml.wolfe.legacy.term.TupleTerm2(a1, a2) => Some(a1, a2)
      case _ => None
    }
  }
  ...
  object Reduce {
    ...
    def unapply(t: Term[Any]) = t match {
      case ml.wolfe.legacy.term.Reduce(op, args) => Some(op, args)
      case _ => None
    }
  }
  ...
  object LambdaAbstraction {
    ...
    def unapply(t:Term[Any]) = t match {
      case ml.wolfe.legacy.term.LambdaAbstraction(s,body) => Some(s,body)
    }
  }
