...
import ml.wolfe.legacy.GradientBasedArgminHint
import ml.wolfe.legacy.value.{MaxValue, Argmin, Argmax}
...
object Compiler {
  ...
  def compile[T](term: Term[T], param: Option[Variable[ml.wolfe.FactorieVector]] = None): Term[T] = {
    ...
    term match {
      ...
      case FunApp(RestrictedFun(Argmin, _, _), LambdaAbstraction(VarSig(w), Annotation(objective, hint: GradientBasedArgminHint))) =>
        val parameter = w.asInstanceOf[Variable[ml.wolfe.FactorieVector]]
