...
import scala.Some
import ml.wolfe.legacy.term.RestrictedFun
import ml.wolfe.legacy.term.DynFunTerm
import ml.wolfe.legacy.value.SeqSet
...
object TermDSL extends ValueDSL {
  ...
  implicit def toRichVec(term: Term[FactorieVector]) = RichVecTerm(term)
...
//  def state[T](map: Map[Variable[T],T]) = State(map.asInstanceOf[Map[Variable[Any],Any]])
  ...
  case class RichVecTerm(term: Term[FactorieVector]) {
    def dot(that: Term[FactorieVector]) = FunApp(vectors.dot, TupleTerm2(term, that))
    def +(that: Term[FactorieVector]) = FunApp(vectors.add, TupleTerm2(term, that))
    def -(that: Term[FactorieVector]) = FunApp(vectors.minus, TupleTerm2(term, that))
  ...
  object vectors extends Constant(Vectors) with ConstantSet[FactorieVector] with HasAdd[FactorieVector] {
    val dot = new Constant(Vectors.Dot) with ConstantFun2[FactorieVector, FactorieVector, Double]
    val add = new Constant(Vectors.VecAdd) with ConstantOperator[FactorieVector]
    val minus = new Constant(Vectors.VecMinus) with ConstantOperator[FactorieVector]
    val unit = new Constant(Vectors.UnitVector) with ConstantFun2[Int, Double, FactorieVector]
    val scalarTimes = new Constant(Vectors.ScalarTimes) with ConstantFun2[FactorieVector, Double, FactorieVector]
  }
  ...
  def min(f:LambdaAbstraction[FactorieVector,Double]) = RichMin(f)
  ...
  case class RichMin(f:LambdaAbstraction[FactorieVector,Double]) {
    def byTrainer(trainerFor: WeightsSet => Trainer = new OnlineTrainer(_, new Perceptron, 5)) = {
      f match {
        case LambdaAbstraction(VarSig(param),objective) if param.domain == vectors =>
          val learned = TrainerBasedMaximization.minimize(param.asInstanceOf[Variable[FactorieVector]],objective,trainerFor)
          MinHelper(
            Term[FactorieVector]( state => learned, Set.empty, param.default),
