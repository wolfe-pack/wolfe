...
import ml.wolfe.legacy.term.State
import ml.wolfe._
import scala.Some
...
object OldChunkingExample {
  ...
  def main(args: Array[String]) {
    ...
    val stream = util.Util.getStreamFromClassPathOrFile("wolfe/datasets/conll2000/train.txt")
    ...
    val sentences = legacy.UtilOld.loadCoNLL(Source.fromInputStream(stream).getLines().take(100), Seq(word, tag, chunk), n)
    ...
    val eval = legacy.Evaluator.evaluate(train, predict)
