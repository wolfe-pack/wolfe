package ml.wolfe.legacy.example

import ml.wolfe.legacy.compiler.{TransformerUtilities, WolfeCompilerPlugin2, WolfeTransformer, StringCompiler}
...
//              global.treeBrowser.browse(t.asInstanceOf[global.Tree])
              ...
              tree
            case outerSum@pat.NumOpApply(sumOp@pat.OpName("ml.wolfe.Wolfe.Operator.Sum", _, _), _, _, _) =>
              println("A sum: " + outerSum)
              tree
            case argminTree@pat.ApplyOperator("ml.wolfe.Wolfe.Operator.Argmin", argmin, dom, obj, List(argminVectorType)) =>
              println("Yeah!")
              println(dom)
              println(obj)
              ...
              obj.asInstanceOf[global.Tree] match {
                case global.Block(_, Function(valDef, body@Apply(Apply(fun, arg1), arg2))) =>
                  ...
                  imp match {
                    case outerSum@pat.NumOpApply(sumOp@pat.OpName("ml.wolfe.Wolfe.Operator.Sum", typeArgs@List(dataType, doubleType), existingSumSelect), data, perInstanceLoss, sumNum) =>
                      //                    case pat.ApplyOperator("ml.wolfe.Wolfe.Operator.Sum", sum, data, perInstanceLoss) =>
                      println("data: " + data)
                        ...
                        case Function(cDef, Apply(logZ, List(Apply(Apply(s, w), List(c))))) =>
                          ...
                          sImp.asInstanceOf[global.Tree] match {
                            case Apply(Select(Apply(rich, List(vectorResult@Apply(feat, _))), dot), List(params)) =>
                              ...
                              val wolfeSym = getModuleByName(newTermName("ml.wolfe.Wolfe"))
                              ...
                              // getClassByName(newTermName("ml.wolfe.Wolfe.Vector"))
                              val vectorType = TransformerUtilities.getClassTypeByName(global, "ml.wolfe.Wolfe.Vector")
                              //val vectorTypeTerm = Select(Select(Ident(newTermName("ml.wolfe")),newTermName("wolfe")),newTypeName("Vector"))
                              val vectorTypeTerm = Select(Select(Ident(newTermName("wolfe")), newTermName("Wolfe")), newTypeName("Vector"))
                              val vectorTypeTerm2 = Select(Ident(newTermName("ml.wolfe.Wolfe")), newTypeName("Vector"))

                              //val sumSelect2 = typed(Select(Select(Ident("ml.wolfe"), "Wolfe"),"sum"))
...
//                              vectorZero
                            ...
                            //                              val scalapplfestTerm = newTermName("ml.wolfe")
                            //                              //todo: ugly hack
                            //                              val vectorType = vectorResult.tpe
                            //
                            //                              println("VectorType: " + vectorType)
                            //
                            //                              val test = global.rootMirror.getPackage(newTermName("ml.wolfe"))
    ...
    val cls = classLoader.loadClass("ml.wolfe.legacy.example.MLEExampleWithLinearModelCompiled") // where className is the name of the class/object in the code
    ...
    object ApplyOperator {
      ...
      def unapply(tree: Any): Option[(String, global.Tree, global.Tree, global.Tree, List[global.Tree])] = tree match {
        case Apply(Apply(TypeApply(s@Select(_, name), types), List(dom)), List(obj))
          if s.symbol.annotations.exists(_.atp.toString().startsWith("ml.wolfe.Wolfe.Operator")) =>
          val annotation = s.symbol.annotations.find(_.atp.toString().startsWith("ml.wolfe.Wolfe.Operator")).get
