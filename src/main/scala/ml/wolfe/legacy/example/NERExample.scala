...
import ml.wolfe.legacy.term.{Max, State, Var, Predicate}
...
import ml.wolfe.legacy.{Evaluator, MathDSL}
...
object SCAI {
  val train = io.Source.fromInputStream(
    util.Util.getStreamFromClassPathOrFile("wolfe/datasets/scai/train.iob"), "iso-8859-1").getLines().take(4)
  val test = io.Source.fromInputStream(
    util.Util.getStreamFromClassPathOrFile("wolfe/datasets/scai/test.iob"), "iso-8859-1").getLines().take(4)
