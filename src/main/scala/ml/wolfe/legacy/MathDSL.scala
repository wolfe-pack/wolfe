...
object MathDSL {
  ...
  implicit def toRichVecPimped(term: Term[FactorieVector]) = RichVecTermPimped(RichVecTerm(term))

  case class RichVecTermPimped(richVecTerm: RichVecTerm) {
    def ∘(that: Term[FactorieVector]) = richVecTerm.dot(that)
  }
  ...
  def Σ[F: Manifest, T: Manifest](seq: Seq[FunApp[F,T]]) = {
    val SeqToVector = new Def[Seq[FunApp[F, FactorieVector]]]
  ...
  def Σ[F: Manifest, T: Manifest](fun: Term[Fun[F,T]]) = {
    val TermFunToVector = new Def[Term[Fun[F, FactorieVector]]]
