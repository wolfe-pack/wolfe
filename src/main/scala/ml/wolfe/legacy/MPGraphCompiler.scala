...
import ml.wolfe.legacy.value.{Fun, Doubles}
import TermDSL._
import ml.wolfe.legacy.term._
import ml.wolfe.legacy.term.Belief
import scala.Some
import ml.wolfe._
...
object MPGraphCompiler {
  ...
  case class Result[T](sig: Sig[T], graph: MPGraph, metaNodes: MetaNodeInfo, metaFactors: MetaFactorInfo) {
    ...
    def printer(index: Option[Index] = Index.toDebug) = new MPGraph.FGPrinter {
      ...
      def factor2String(factor: Factor) = metaFactors.forCoeff.get(factor).orElse(metaFactors.forBase.get(factor))
        .get.toString
      def vector2String(vector: ml.wolfe.FactorieVector) = index.map(_.vectorToString(vector,
  ...
  def compile[T](sig: Sig[T], term: Term[Double],
    ...
    val coeffFactors =
      if (isZero)
        new Array[(Factor, Term[FactorieVector])](0)
   ...
   * @param mpGraph the graph to add to.
   */
  def compileCoefficientTerm(baseTerm: Term[FactorieVector], metaNodes: MPGraphCompiler.MetaNodeInfo,
