...
import scala.Some
import ml.wolfe.legacy.term.ImageSeq1
import ml.wolfe.legacy.term.LambdaAbstraction
import ml.wolfe.legacy.term.SeqTerm
  ...
  def groupLambdasOnce[T](term: Term[T], filter: Variable[Any] => Boolean = x => true): Term[T] = {
    ...
    term match {
      case vectors.add.Reduced(SeqTerm(args)) =>
        val merged = args.foldLeft(Seq.empty[Term[FactorieVector]])(mergeOneTerm(_, _, vectors.add))
