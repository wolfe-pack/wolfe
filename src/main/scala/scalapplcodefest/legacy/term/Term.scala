 ...
 * [[scalapplcodefest.legacy.term.State]] (or "environment") which assigns free variables to objects in a domain,
 * a term itself denotes an object with Type `T` of that domain. In other words, the meaning of a term is a mapping from
 * [[scalapplcodefest.legacy.term.State]] objects to objects of type `T`. Usually this function is recursively defined through
