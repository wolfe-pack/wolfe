 ...
 * @param superRange a super range (everything outside of this range can never be returned)
 * @tparam A type of arguments to function.
 * @tparam B type of return values of function.
 */
case class ConstantFun[A, B](function: Fun[A, B], superDomain: Term[Set[A]], superRange: Term[Set[B]]) extends FunTerm[A, B] {

  val restrictedFunction = new Fun[A, B] {
    def apply(v1: A) = function(v1)
    def isDefinedAt(x: A) = {
      val d = superDomain.eval(State.empty)
      val r = superRange.eval(State.empty)
      d.isDefined && function.isDefinedAt(x) && d.get(x) && r.isDefined && r.get(function(x))
    }
  }
  def eval(state: State) = Some(restrictedFunction)
  def variables = superDomain.variables ++ superRange.variables
...
case class LambdaAbstraction[A, B](variable: Variable[A], term: Term[B]) extends FunTerm[A, B]  {
  ...
  def superRange = term.domain[B]
  def eval(state: State) = {
    for (r <- superRange.eval(state); d <- superDomain.eval(state)) yield new Fun[A, B] {
...
trait UncurriedLambdaAbstraction[A1,R] {
  ...
  def superRange = lambdaLast.superRange
}
