...
class AdagradSpecs extends WolfeSpec {
  ...
  "Adagrad" should {
    "optimize a quadratic objective" in {
      ...
      val result = argmax(Doubles) { x => obj(x).argmaxBy(Argmaxer.adaGrad2(AdaGradParameters(100, 1)))}
    ...
    "optimize a multivariate quadratic objective" in {
      ...
      val result = argmax(X) { x => obj(x).argmaxBy(Argmaxer.adaGrad2(AdaGradParameters(100, 1)))}
