...
class AdagradSpecs extends WolfeSpec {
  ...
  "Adagrad" should {
    "optimize a quadratic objective" in {
      ...
      val result = argmax2(Doubles) { x => obj(x).argmaxBy(ArgmaxerImpl.adaGrad2(AdaGradParameters(100, 1)))}
    ...
    "optimize a multivariate quadratic objective" in {
      ...
      val result = argmax2(X) { x => obj(x).argmaxBy(ArgmaxerImpl.adaGrad2(AdaGradParameters(100, 1)))}
