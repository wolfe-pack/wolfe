...
class VarSeqSumSpecs extends WolfeSpec {
  ...
  "A sum over variable length sequences" should {
    "evaluate to the sum of all its arguments when the sequence is constructed" ignore {
      val n = 3
    ...
    "evaluate to the sum of all its arguments when the sequence is a variable" ignore {
      val n = 3
      val xs = varSeqs(doubles,0,n)
      val length = xs.lengthDom.Var
      val d = doubles.Var
    ...
    "calculate the gradient for constructed sequences" ignore {
      val n = 3
