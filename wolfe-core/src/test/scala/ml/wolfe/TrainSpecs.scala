...
class TrainSpecs extends WolfeSpec {
  ...
  def sentences = Wolfe.all(Sentence)(seqs(
    Wolfe.all(Token)(strings x Seq(Tag('DT), Tag('NN)) x Seq(Chunk('?)))
  ))
  ...
  def predictor(w: Vector)(s: Sentence) = argmax(sentences where evidence(obs)(s)) { model(w) }
