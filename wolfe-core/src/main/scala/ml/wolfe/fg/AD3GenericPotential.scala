...
trait AD3GenericPotential extends DiscretePotential {
  ...
  private def addToActiveSet(r:Int): Unit = {
    ...
    Ainv = DenseMatrix.vertcat(
      DenseMatrix.horzcat(X1, X2.toDenseMatrix),
      DenseMatrix.horzcat(X3.t.toDenseMatrix.t, new DenseMatrix[Double](1, 1, Array(q)))
    )
    ...
    for(i <- (0 until n+2).optimized) { val x = Ainv(i, n+2); Ainv(i, n+2) = Ainv(i, n+1); Ainv(i, n+1) = x }
    for(i <- (0 until n+2).optimized) { val x = Ainv(n+2, i); Ainv(n+2, i) = Ainv(n+1, i); Ainv(n+1, i) = x }
  ...
  private def removeFromActiveSet(r:Int): Unit = {
    ...
    val n = activeSet.length
    for(i <- (0 until n+1).optimized) { val x = Ainv(i, n+1); Ainv(i, n+1) = Ainv(i, r); Ainv(i, r) = x }
    for(i <- (0 until n+1).optimized) { val x = Ainv(n+1, i); Ainv(n+1, i) = Ainv(r, i); Ainv(r, i) = x }

    val E = Ainv(0 until n, 0 until n)
    val f:DenseVector[Double] = Ainv(0 until n, n+1)
    val g:DenseVector[Double] = Ainv(n+1, 0 until n).t
    val h = Ainv(n+1, n+1)
    Ainv = E - (f * g.t) / h
